...
const random_damage_multi = (rarity: string): number => {
  ...
  if (rarity === "COMMON") {
    damage_multi_by_rarity = parseFloat(
      Phaser.Math.FloatBetween(0.2, 0.8).toFixed(1),
    );
  } else if (rarity === "RARE") {
    damage_multi_by_rarity = parseFloat(
      Phaser.Math.FloatBetween(0.9, 1.6).toFixed(1),
    );
  } else if (rarity === "EPIC") {
    damage_multi_by_rarity = parseFloat(
      Phaser.Math.FloatBetween(1.7, 2.5).toFixed(1),
    );
  } else if (rarity === "LEGENDARY") {
    damage_multi_by_rarity = parseFloat(
      Phaser.Math.FloatBetween(2.5, 2.8).toFixed(1),
    );
  }
...
export class Weapon {
  ...
  get damage() {
    return (
      parseFloat((this._damage * this._damage_multi).toFixed(2)) + this._damage
    );
  }
